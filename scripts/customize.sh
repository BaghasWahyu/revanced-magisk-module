...
if [ -n "$BASEPATH" ] && cmpr $BASEPATH $MODPATH/__PKGNAME.apk; then
	...
	rm $MODPATH/__PKGNAME.apk
else
	ui_print "* Updating stock __PKGNAME"
	mv -f $MODPATH/__PKGNAME.apk /data/local/tmp/__PKGNAME.apk
	chcon u:object_r:apk_data_file:s0 /data/local/tmp/__PKGNAME.apk
	if ! op=$(pm install -i com.android.vending -r -d /data/local/tmp/__PKGNAME.apk 2>&1); then
		ui_print "ERROR: APK installation failed!"
		abort "${op}"
	fi
	rm /data/local/tmp/__PKGNAME.apk
	BASEPATH=$(basepath)
fi
...
rm -r $MODPATH/bin $MODPATH/lib $MODPATH/rv.patch
